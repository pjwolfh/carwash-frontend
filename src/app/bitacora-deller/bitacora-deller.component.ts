...
export class BitacoraDellerComponent implements OnInit {
  ...
  obtenerDeller() {
    this.http.get(`http://localhost:3000/api/dellers/${this.idDeller}`).subscribe((data: any) => {
      this.deller = data;
    });
  }

  obtenerBitacoras() {
    this.http.get<any[]>(`http://localhost:3000/api/dellers/bitacora/${this.idDeller}`).subscribe(data => {
      this.bitacoras = data;
    });
  }
...
async onIndividualFileChange(event: Event, index: number) {
  ...
  guardarBitacora() {
    ...
    this.http.post('http://localhost:3000/api/dellers/bitacora/agregar', formData).subscribe({
      next: () => {
  ...
  eliminarBitacora(bitacora: any) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.http.delete(`http://localhost:3000/api/dellers/bitacora/${bitacora.id_bitacora}`).subscribe({
          next: () => {
